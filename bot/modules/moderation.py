...
async def delete_messages(message: types.Message, count: int = 1):
    ...
    time.sleep(3) # Небольшая пауза перед удалением уведомления (в асинхронном коде лучше использовать asyncio.sleep)
    # Но так как мы в асинхронной функции, используем asyncio.sleep
    import asyncio
