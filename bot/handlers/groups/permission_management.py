...
@router.message(F.text.regexp(r'^[.!/]права лист', re.IGNORECASE), RankFilter(min_rank=5))
...
@router.message(F.text.regexp(r'^[.!/]права\s+\d+\s*=\s*\d+', re.IGNORECASE), RankFilter(min_rank=5))
async def handle_permission_change(message: types.Message):
    ...
    await message.reply(f"✅ Права изменены!\nДействие: <b>{action['name']}</b>\nМинимальный ранг теперь: <b>{RANKS[new_rank]}</b> ({new_rank})")
